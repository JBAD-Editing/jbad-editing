...
$("joinButton").onclick = () => {
  if (!configured) {
    alert("Jbad Editing is not connected to Supabase yet. Open supabase-config.js, add your Project URL and public/publishable key, save it, then reload the site.");
    return;
  }
  setAuthMode("signup");
  showModal(authModal);
...
$("forgotPassword").onclick = async () => {
  ...
  if (error) {
    return $("authMessage").textContent = error.message;
  }
...
$("resetPasswordForm").addEventListener("submit", async (e) => {
  ...
  if (error) {
    return $("resetPasswordMessage").textContent = error.message;
  }
...
async function refreshAuthButton(){
  ...
  $("authButton").textContent =
    session ? "Editor dashboard" : "Editor login";
}
...
async function loadPublicContent(){
  if (!configured) {
    $("editorsGrid").innerHTML =
      `<div class="empty-state">Supabase is not connected yet. Add your keys to <b>supabase-config.js</b>.</div>`;
  ...
  if (profileError) {
    $("editorsGrid").innerHTML =
      `<div class="empty-state">${escapeHtml(profileError.message)}</div>`;
  } else {
  ...
  if (postError) {
    $("featuredGrid").innerHTML =
      `<div class="empty-state">${escapeHtml(postError.message)}</div>`;
  } else {
...
function editorCard(p){
  const initials = escapeHtml(
    (p.display_name || "J").slice(0,1).toUpperCase()
  );

  return `<article class="editor-card">
    <div class="editor-avatar">
      ${p.avatar_url
        ? `<img src="${safeUrl(p.avatar_url)}" alt="" style="width:100%;height:100%;object-fit:cover">`
        : initials}
    </div>
    <div class="editor-info">
      ...
      ${p.website_url
        ? `<a class="small" target="_blank" rel="noopener" href="${safeUrl(p.website_url)}">Portfolio ↗</a>`
        : ""}
    </div>
  </article>`;
}
...
async function openDashboard(){
  ...
  $("dashboardContent").innerHTML = `
    <form id="profileForm" class="dashboard-grid">
      <label>
        Display name
        <input id="dName" value="${escapeAttr(profile?.display_name || "")}" maxlength="80" required>
      </label>

      <label>
        Speciality
        <input id="dSpeciality" value="${escapeAttr(profile?.speciality || "")}" maxlength="120">
      </label>

      <label class="full-row">
        Bio
        <textarea id="dBio" maxlength="500">${escapeHtml(profile?.bio || "")}</textarea>
      </label>

      <label>
        Website / portfolio URL
        <input id="dWebsite" type="url" value="${escapeAttr(profile?.website_url || "")}" placeholder="https://…">
      </label>

      <label>
        Avatar URL
        <input id="dAvatar" type="url" value="${escapeAttr(profile?.avatar_url || "")}" placeholder="https://…">
      </label>
    ...
    <form id="postForm" class="dashboard-grid">
      <label>
        Title
        <input id="postTitle" maxlength="100" required placeholder="e.g. Football montage">
      </label>

      <label>
        Type
        <select id="postType" style="padding:12px;border:1px solid #ccc;border-radius:8px">
      ...
      <label class="full-row">
        Description
        <textarea id="postDescription" maxlength="500" placeholder="Tell people about the edit"></textarea>
      </label>

      <label class="full-row">
        Media file
        <input id="postFile" type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime" required>
      </label>
        ...
        ? posts.map(p =>
          `<div class="post-row">
            <div>
              ...
              <div class="small">${escapeHtml(p.media_type)}</div>
            </div>
            <button class="button danger delete-post" data-id="${p.id}" data-url="${escapeAttr(p.media_url)}">Delete</button>
          </div>`
        ).join("")
        : `<p class="muted">You haven't published anything yet.</p>`}
    </div>
  `;
  ...
  $("postForm").onsubmit = async (e) => {
    ...
    if (type === "image" && !file.type.startsWith("image/")) {
      return message.textContent = "Choose an image file.";
    }

    if (type === "video" && !file.type.startsWith("video/")) {
      return message.textContent = "Choose a video file.";
    }
    ...
    if (uploadError) {
      return message.textContent = uploadError.message;
    }
    ...
    const { error: insertError } =
      await supabase.from("editor_posts").insert({
...
// If Supabase redirects back here after a password-recovery email,
// the client emits PASSWORD_RECOVERY and the reset form above
// is shown automatically.
if (configured) {
  supabase.auth.getSession().catch(
    (error) => console.error("Jbad auth session error:", error)
  );
}
